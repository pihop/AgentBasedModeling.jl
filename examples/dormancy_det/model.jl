...
sample_gamma(α, β, age) = rand(truncated(Gamma(1/β, α*β), lower=age))
...
@register_symbolic death_time(T1, T2, α, τ, time)
@register_symbolic env_step(T1, T2, time)
...
die = @interaction begin
    @channel death_time($T1, $T2, $αdeath, $Cτ, $t), $C --> 0 
